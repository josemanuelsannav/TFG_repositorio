...
namespace CityFlow
{
    ...
    void Vehicle::update()
    { // TODO: use something like reflection?
        if (buffer.isEndSet)
        {
    ...
    void Vehicle::updateLeaderAndGap(Vehicle *leader)
    {
        ...
        else
        {
            ...
            for (int i = 0;; ++i)
            {
                ...
                { // if laneLink, check all laneLink start from previous lane, because lanelinks may overlap
    ...
    int Vehicle::getReachSteps(double distance, double targetSpeed, double acc) const
    {
        ...
        if (distanceUntilTargetSpeed > distance)
        {
            return std::ceil((std::sqrt(
                                  vehicleInfo.speed * vehicleInfo.speed + 2 * acc * distance) -
                              vehicleInfo.speed) /
                             acc / interval);
        }
        else
        ...
        for (auto cebra : engine->getCebras())
        {
            ...
            else
            {
                if (((cebra.pos_x - 24 <= this->getPoint().x && cebra.pos_x - 12 >= this->getPoint().x && cebra.pos_y - 12 <= this->getPoint().y && cebra.pos_y >= this->getPoint().y) ||
                     ...
                     (cebra.pos_x + 12 <= this->getPoint().x && cebra.pos_x + 24 >= this->getPoint().x && cebra.pos_y <= this->getPoint().y && cebra.pos_y + 12 >= this->getPoint().y) ||
                     (cebra.pos_x - 12 <= this->getPoint().x && cebra.pos_x >= this->getPoint().x && cebra.pos_y + 12 <= this->getPoint().y && cebra.pos_y + 24 >= this->getPoint().y)) &&
                    (this->getLeader() != nullptr && this->getLeader()->estaJustoDespuesDeCebra() == true))
                {

                    if (this->getLeader()->getLeader() != nullptr && (this->getLeader()->getLeader()->getSpeed() < this->getSpeed() || this->getLeader()->getSpeed() == 0.0))
                    {
                        v = 0.0;
                    }
                }
    ...
    bool Vehicle::estaJustoDespuesDeCebra()
    {
        for (auto cebra : engine->getCebras())
        {
            // if (cebra.is_activated == true)
            ...
            if ((cebra.pos_x - 24 <= this->getPoint().x && cebra.pos_x >= this->getPoint().x && cebra.pos_y <= this->getPoint().y && cebra.pos_y + 12 >= this->getPoint().y) ||
                ...
                (cebra.pos_x <= this->getPoint().x && cebra.pos_x + 12 >= this->getPoint().x && cebra.pos_y <= this->getPoint().y && cebra.pos_y + 24 >= this->getPoint().y))
            {
                return true;
            }
            // }
    ...
    bool Vehicle::estaEnStop()
    {
        for (auto stop : engine->getStops())
        {
            if (this->getCurLane() != nullptr)
            {
                ...
                if (road != nullptr)
                {
                    ...
                    {
                        // Tu código aquí
                        if ((this->getPoint().x >= stop.pos_x - 23 && this->getPoint().x <= stop.pos_x + 23) && (this->getPoint().y <= stop.pos_y + 23 && this->getPoint().y >= stop.pos_y - 23))
                        {
                            return true;
                        }
                    }
   ...
   bool Vehicle::estaEncimaCeda(){
    ...
    bool Vehicle::hayCochesViniendo()
    {
        ...
        const Intersection intersection = this->getCurLane()->getBelongRoad()->getEndIntersection();
        ...
        for (auto road : roads)
        {
            ...
            for (auto sinStop : engine->getNotStops())
            {
                if (road->getId() == sinStop.road)
                {
    ...
    bool Vehicle::calleConStop()
    {
        for (auto stop : engine->getStops())
        {
            if (stop.road == this->getCurLane()->getBelongRoad()->getId())
            {
                return true;
            }
        }
    ...
    bool Vehicle::calleSinStop()
    {
        ...
        for (auto notstop : engine->getNotStops())
        {
            if (notstop.road == this->getCurLane()->getBelongRoad()->getId())
            {
                return true;
            }
        }
