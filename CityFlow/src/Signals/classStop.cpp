...
namespace CityFlow
{
    ClassStop::ClassStop(const std::string &id, const std::string &road, int pos_x, int pos_y, const std::string &direccion)
        : id(id), road(road), pos_x(pos_x), pos_y(pos_y), direccion(direccion) {}

    double ClassStop::affectVehicle(Vehicle &vehicle, double v)
    {
        if (estaEnStop(vehicle))
        {
            if (vehicle.contadorStop == 5)
            {
    ...
    void ClassStop::addEngine(Engine *engine)
    {
        this->engine = engine;
    }

    bool ClassStop::estaEnStop(Vehicle &vehicle)
    {
        if (vehicle.getCurLane() != nullptr)
        {
            ...
            {
                if (vehicle.getPoint().x >= this->pos_x - 23 && vehicle.getPoint().x <= this->pos_x + 23 && vehicle.getPoint().y >= this->pos_y - 23 && vehicle.getPoint().y <= this->pos_y + 23)
                {
                    return true;
                }
            }
    ...
    bool ClassStop::hayCochesViniendo(Vehicle &vehicle)
    {
